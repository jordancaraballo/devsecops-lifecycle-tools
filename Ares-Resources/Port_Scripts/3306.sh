...
if [ $runTest == true ]; then
        ...
        echo "------------------------------- Nmap Scan -------------------------------------"
        nmap -sV -Pn $1 -p 3306 --script mysql-audit,mysql-databases,mysql-dump-hashes,mysql-empty-password,mysql-enum,mysql-info,mysql-query,mysql-users,mysql-variables,mysql-vuln-cve2012-2122
        echo ""
        echo "------------------------------ Banner Grab ------------------------------------"
        nmap -sV -p 3306 --script=banner $1
   else
